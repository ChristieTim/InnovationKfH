class InnovationsController < ApplicationController
  ...
  def index
    ...
    @innovations = Innovation.search(params[:term])
  end
  ...
  private
    ...
    def innovation_params
      params.require(:innovation).permit(:date, :author, :title, :orgaim, :tags, :summary, :lksSearch, :context, :aimsobs, :methods, :collaboration, :ip, :learningpoints, :term)
    end
end
